...
var maxFees = 0.0026;
var key = '9B29SWO5sFX5PaTD3jia4SugBP/NhhlbEppxznAjxD9uFy95MYAoMBV/';
var secret = 'J0t5AqfdGX6mU6gajLKmAMBQSUHq0jmCXNmeR//8Ohq8/06CTqVN7LlqXFrOBbTnFhv1WVNoJNFAb+3+K+6e7g==';
var kraken = new KrakenClient(key, secret);
var seqs = JSON.parse(fs.readFileSync('./sequences.json', 'utf8'));
var minVolume = JSON.parse(fs.readFileSync('./minimum_volumes.json', 'utf8'));
...
function start() {
    ...
    prompt('## Numero de sequence: ', function (input) {
        ...
        seq = seqs[input];
        console.log(seq);
        var initPair = seq['pairs'][0];
        var initDevice = seq['codes'][0] === '1' ? getDeviceFromPair(initPair, 1) : getDeviceFromPair(initPair, 0);
...
function order(initVol, initDevice, seq) {
    ...
    kraken.api('Ticker', { "pair": 'BCHEUR,BCHUSD,BCHXBT,DASHEUR,DASHUSD,DASHXBT,EOSETH,EOSXBT,GNOETH,GNOXBT,ETCETH,ETCXBT,ETCEUR,ETCUSD,ETHXBT,ETHCAD,ETHEUR,ETHGBP,ETHJPY,ETHUSD,ICNETH,ICNXBT,LTCXBT,LTCEUR,LTCUSD,MLNETH,MLNXBT,REPETH,REPXBT,REPEUR,XBTCAD,XBTEUR,XBTGBP,XBTJPY,XBTUSD,XMRXBT,XMREUR,XMRUSD,XRPXBT,XRPEUR,XRPUSD,ZECXBT,ZECEUR,ZECUSD' }, function (error, response) {
        ...
        if (error) {
            ...
            var benef = (vol[3] - vol[0]) / vol[0] * 100;
